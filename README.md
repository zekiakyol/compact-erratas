## Compact Erratas

- This repository provides compact and reader-friendly erratas in PDF format by avoiding large page margins, oversized fonts, lengthy acknowledgments, web-only accessibility, and low resolution.
- The books in this repository are generally selected from [Proof-Based Math Readings](https://github.com/zekiakyol/proof-based-math-readings).
- Each errata includes a link to the original document it's based on. Thanks to the authors of the original documents!

## Erratas

:date: Errata Version <br> (Y/M/D) | :scroll: Errata
:--: | :--:
2008-01-07 | [Microeconometrics: Methods and Applications <br> - A. Colin Cameron, Pravin K. Trivedi (2005)](https://github.com/zekiakyol/compact-erratas/blob/main/Microeconometrics_ColinCameronPravinTrevedi_Errata.pdf)
2026-05-09 | [Linear Algebra Done Right <br> - Sheldon Axler (4th Edition, 2024)](https://github.com/zekiakyol/compact-erratas/blob/main/LinearAlgebraDoneRight_SheldonAxler_Errata.pdf)
2021-07-20 | [Elements of Large-Sample Theory <br> - Erich L. Lehmann (1999)](https://github.com/zekiakyol/compact-erratas/blob/main/ElementsOfLargeSampleTheory_ErichLehmann_Errata.pdf)
2014-10-07 | [Matrix Algebra <br> - Karim M. Abadir, Jan R. Magnus (2005)](https://github.com/zekiakyol/compact-erratas/blob/main/MatrixAlgebra_KarimAbadirJanMagnus_Errata.pdf)
